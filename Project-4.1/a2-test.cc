...
void test1 (int option, int runlen, int selectedTable) {
    ...
    BigQ bq (input, output, sortorder, runlen, rel->path());

    pthread_t threadID;
    int rs = pthread_create(&threadID, NULL, BigQ::SortAndDumpToPipe, (void *)&bq);
    if (rs)
    {
