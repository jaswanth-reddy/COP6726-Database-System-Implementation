...
char *tpch_dir = "tpch-dbgen/"; // dir where dbgen tpch files (extension *.tpch-dbgen) can be found
...
void test2() {
    ...
    dbfile.MoveFirst(); // getting the first page
    ...
    while (dbfile.GetNext(temp) == 1)
    {
        ...
        if (counter % 10000 == 0)
            cout << "\nRecord count = " << counter << std::endl;
    }
    cout << "Scanned " << counter << " recs" << std::endl;
    dbfile.Close();
}
